...
if blkid /dev/vdb2 2>/dev/null | grep -q btrfs; then
    ...
    echo "=== Updating OS configuration on subvol=@ (systemd-boot + yay + /efi mount) ==="
    ...
    mkdir -p /mnt/target-root/efi
    mount /dev/vdb1 /mnt/target-root/efi || true
    mkdir -p /mnt/target-root/efi/loader/entries
    sed -i 's|\([ \t]\)/boot\([ \t]\)|\1/efi\2|' /mnt/target-root/etc/fstab
    umount /mnt/target-root/efi 2>/dev/null || true
    mkdir -p /mnt/target-root/usr/local/bin
    cat <<'EOF' > /mnt/target-root/usr/local/bin/yay
#!/bin/bash
exit 0
EOF
    chmod +x /mnt/target-root/usr/local/bin/yay
    sync
    umount /mnt/target-root
...
mkdir -p /mnt/target/{efi,home,var/log,var/cache,var/tmp,srv,root}
mount /dev/vdb1 /mnt/target/efi
...
chroot /mnt/target pacman -Sy --noconfirm systemd-boot efibootmgr dialog snapper btrfs-progs btrbk borg rclone age zstd shellcheck make pv zenity
chroot /mnt/target bootctl --path=/efi install
# Clean up GRUB remnants from base cloud image
rm -rf /mnt/target/etc/default/grub /mnt/target/boot/grub
# Ensure systemd-boot loader entries directory exists on ESP
mkdir -p /mnt/target/efi/loader/entries

# Mock AUR helper for EndeavourOS
cat <<'EOF' > /mnt/target/usr/local/bin/yay
#!/bin/bash
exit 0
EOF
chmod +x /mnt/target/usr/local/bin/yay
